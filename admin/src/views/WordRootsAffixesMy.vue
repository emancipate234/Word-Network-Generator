    ...
    <div>
        ...
        <el-dialog title="编辑" :visible.sync="editDialog" width="30%">
            <el-form ref="editFrom" :model="editFrom" :rules="rules" @keyup.enter.native="update()"
                     label-width="100px">
                <el-form-item label="单词" prop="wordId">
                    <el-select v-model="editFrom.wordId" filterable placeholder="请选择单词">
                        <el-option v-for="(item,index) in wordList" :key="item.id"
                                   :value="item.id" :label="item.name">
                        </el-option>
                    </el-select>
                ...
                <el-form-item label="词根" prop="root">
                    <el-input v-model="editFrom.root" placeholder="请输入词根"></el-input>
                </el-form-item>
                <el-form-item label="词缀" prop="affixe">
                    <el-input v-model="editFrom.affixe" placeholder="请输入词缀"></el-input>
                </el-form-item>
                <el-form-item label="是否公开" prop="isOpen">
        ...
        <el-dialog title="明细" :visible.sync="detailDialog" width="60%">
            <div class="echart" id="echart" style="width: 100%;height: 300px;"></div>
            <span slot="footer" class="dialog-footer">
    ...
    export default {
        ...
        methods: {
            ...
            handleDetail(index, row){
                ...
                this.getRequest('/wordRootsAffixes/getDetailById?id=' + row.id).then(resp => {
                    console.log(resp.obj)
                    let detail = resp.obj
                    let data = {
                        name: detail.word.name,
                        children: [{
                            name: detail.root,
                            children: detail.rootList
                        },
                        {
                            name: detail.affixe,
                            children: detail.affixeList
                        }]
                    }
                    _this.initChart("echart", data)
                })
            },
            initChart(item, data) {
            ...
            update() {
                ...
                this.$refs['editFrom'].validate(valid => {
                    if (valid) {
                        this.putRequest('/wordRootsAffixes/update', this.editFrom).then(resp => {
                            if (resp) {
                                this.initData()
